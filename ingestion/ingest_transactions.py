...
from schemas import TRANSACTIONS_SCHEMA, validate_schema
from copy_utils import copy_csv_to_table

logger = get_logger("ingest_transactions")
engine = create_engine(DB_URL)

FILE_NAME = "transactions_v2.csv"
TABLE_NAME = "transactions"
SCHEMA_NAME = "raw"

def validate_mode():
    file_path = f"{DATA_PATH}/{FILE_NAME}"
    logger.info("VALIDATE mode: raw.transactions")
    
    for i, chunk in enumerate(pd.read_csv(file_path, chunksize=CHUNK_SIZE)):
        validate_schema(chunk, TRANSACTIONS_SCHEMA, "raw.transactions")
...
def copy_mode():
    logger.info("COPY mode: raw.transactions")
    copy_csv_to_table(
        file_path=f"{DATA_PATH}/{FILE_NAME}",
        schema=SCHEMA_NAME,
        table=TABLE_NAME,
